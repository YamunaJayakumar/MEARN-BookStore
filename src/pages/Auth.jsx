...
function Auth({ insideRegister }) {
  ...
  const handleLogin = async (e) => {
    ...
    if (email && password) {
      ...
      toast.info('please fillt eh form completely')
    }


  }
  const handleGoogleLogin=async(credentialResponse)=>{
    console.log("insidehandleGoogleLogin");
     console.log(credentialResponse);
     const decode =jwtDecode(credentialResponse.credential)
     console.log(decode);
     //email,password,picture
     const result=await googleLoginAPI({username:decode.name,email:decode.email,password:'googlePassword',profilepic:decode.picture})
     if(result.status==200){
      toast.success("Login Successfull")
          sessionStorage.setItem("token", result.data.token)
          sessionStorage.setItem("user", JSON.stringify(result.data.user))
